...
function StatsDelay({statsData}) {
    ...
    const getStats = async (statsData) => {
        ...
        groupedByDelayRange.forEach((values, keys) => {
            ...
            delayByRange.push({ index: countOfByDelayRange, trainNames: trainNames, delayRange: averageDelay, countOfTrainsByDelayRange: countOfTrainsByDelayRange, label: keys + "min\n" + countOfTrainsByDelayRange + " trains" })
        });
    ...
    return (
        <div>
            <div className="divide-y divide-gray-200 overflow-hidden rounded-lg bg-white shadow my-8">
                ...
                <div className="px-4 py-5 sm:p-6">
                    ...
                    <div ref={ref} className="grid place-content-center">
                        <VictoryChart
                            width={typeof (width) == "number" ? width - 30 : 200}
            ...
            <ul role="list" className="divide-y divide-gray-100">
                {dataByFilterTrainNumber.map((train) => (
                    <li key={train.trainName} className="flex justify-between gap-x-6 py-5">
                        <div className="flex min-w-0 gap-x-4">
                            <p className="text-sm font-semibold leading-6 text-gray-900">{train.trainName}</p>
                        </div>
                        <div className="hidden shrink-0 sm:flex sm:flex-col sm:items-end">
                            <p className="text-sm leading-6 text-gray-900">{Math.round(train.averageDelayByTrainNumber)}</p>
                        </div>
                    </li>
                ))}
            </ul>
